...
import { AlertTriangle, CheckCircle, Download, Info, Loader2, Save, Share2 } from "lucide-react"
...
export function NewsAnalyzer() {
  ...
  const extractKeywords = (text: string): string[] => {
    ...
    return uniqueWords.sort(() => Math.random() - 0.5).slice(0, Math.min(8, uniqueWords.length))
  }
  ...
  const exportResult = () => {
    ...
    downloadAnchorNode.setAttribute("download", `news-analysis-${new Date().toISOString().slice(0, 10)}.json`)
    ...
    <div className="space-y-6">
      <div className="grid md:grid-cols-3 gap-4">
        <div className="md:col-span-2">
          <Label htmlFor="news-text" className="block text-sm font-medium mb-2 text-slate-700 dark:text-slate-300">
        ...
        <div>
          ...
          <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-md">
            <h4 className="text-sm font-medium mb-2 text-blue-700 dark:text-blue-300 flex items-center">
      ...
      <div className="flex flex-wrap gap-2">
        <Button onClick={analyzeNews} disabled={isAnalyzing || !newsText.trim()} className="flex-1">
          {isAnalyzing ? (
            <>
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="outline" onClick={saveToHistory} disabled={!result}>
        ...
        <Card className="mt-6">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center">
                ...
                <h3 className="text-xl font-semibold">
                  {result.prediction === "FAKE" ? "Potentially Fake News" : "Likely Authentic News"}
                </h3>
              </div>
              <Badge variant={result.prediction === "FAKE" ? "destructive" : "default"}>{result.prediction}</Badge>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-sm font-medium">{Math.round(result.probability * 100)}%</span>
                  </div>
                  <Progress
                ...
                <div>
                  <div className="flex justify-between mb-1">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Source Credibility</span>
                    <span className="text-sm font-medium">{Math.round(result.sourceCredibility * 100)}%</span>
                  </div>
                  <Progress
                    value={result.sourceCredibility * 100}
                    className={result.sourceCredibility > 0.5 ? "text-green-500" : "text-amber-500"}
                  />
                </div>

                <div>
                  <div className="flex justify-between mb-1">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Factual Consistency</span>
                    <span className="text-sm font-medium">{Math.round(result.factualConsistency * 100)}%</span>
                  </div>
                  <Progress
                    value={result.factualConsistency * 100}
                    className={result.factualConsistency > 0.6 ? "text-green-500" : "text-amber-500"}
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {Object.entries(result.emotionalTone).map(([emotion, value]) => (
              ...
              <div className="space-y-4">
                <div>
                  <h4 className="text-sm font-medium mb-2 text-slate-700 dark:text-slate-300">Sentiment Analysis</h4>
                ...
                <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-md">
                  ...
                  <KeywordHighlighter text={newsText} keywords={result.keywords} />
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-md">
              <h4 className="text-sm font-medium mb-2 text-slate-700 dark:text-slate-300">Analysis Summary</h4>
              <p className="text-sm text-slate-600 dark:text-slate-400">
                {result.prediction === "FAKE"
                  ? `This content shows several indicators of potentially misleading information. The emotional tone is heightened${result.emotionalTone.fear > 0.5 ? ", particularly in fear and surprise," : ""} and the factual consistency score is low. The source credibility analysis suggests caution when sharing this information.`
