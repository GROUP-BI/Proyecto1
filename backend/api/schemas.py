...
class PredictionOutput(BaseModel):
    ...
    keywords: List[str] = Field(..., example=["news", "policy", "election"])
...
class PredictionResponse(BaseModel):
    ...
    model_version: Optional[str] = Field(
        None, example="20250327-140000"
    )  # Podrías añadir versión/timestamp
...
class RetrainResponse(BaseModel):
    ...
    precision: Optional[float] = Field(
        None, ge=0.0, le=1.0
    )  # Métricas pueden ser None si falla
    recall: Optional[float] = Field(None, ge=0.0, le=1.0)
    f1_score: Optional[float] = Field(None, ge=0.0, le=1.0)
    new_model_version: Optional[str] = Field(
        None, example="20250327-150000"
    )  # Versión/timestamp del nuevo modelo
